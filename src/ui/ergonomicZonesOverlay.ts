...
import { UI, UI_DEPTH, designSize, pixelText, dashedArc } from './uiTheme';

/**
 * Col D reference overlay: the right-hand thumb-reach diagram. Three translucent
 * horizontal bands (avoid / stretch / thumb) plus dashed reach arcs centered on
 * the bottom-right thumb pivot. Pure design aid — HIDDEN by default and never
 * interactive, so it cannot steal pointer input from the live HUD beneath it.
 */
export class ErgonomicZonesOverlay {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    // Translucent zone bands (top -> bottom). Origin top-left for clean spans.
    ...
    // Dashed reach arcs from the bottom-right thumb pivot (632,1230).
    ...
    // Zone labels + thumb-friendly hint.
    this.root.add([
      pixelText(scene, cx, 150, '避免高满点击区(不需操作)', {
        size: 20,
